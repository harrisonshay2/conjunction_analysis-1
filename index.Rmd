...
#   summarise(avgConseq = mean(conseq, na.rm=T),
#             avgConseq_s = mean(conseq_s, na.rm=T),
#             avgCM = mean(combinedMass, na.rm=T),
#             avgCM_s = mean(combinedMass_s, na.rm=T),
#             avgPers = mean(persistence),
#             avgPers_s = mean(persistence_s),
#             avgSD = mean(SD),
#             avgSD_s = mean(SD_s),
#             avgRisk = mean(risk, na.rm=T),
#             avgRisk_s = mean(risk_s, na.rm=T),
#             totRisk = sum(risk, na.rm=T),
#             totRisk_s = sum(risk_s, na.rm=T)) %>%
#   #kable(digits = 1, format.args = list(big.mark=","))
#   write.table(file=clip, sep = '\t', row.names = FALSE)
# close(clip)
# 
# all_conjs %>% dplyr::select(c(combinedMass_s,persistence_s, SD_s)) %>%
#   summarise_all(list(min=min, max=max, mean=mean, sd=sd), na.rm=T) %>%
#   write.table(file=clip, sep = '\t', row.names = FALSE)
# 
# all_conjs %>% arrange(desc(risk)) %>%
#   head(50) %>%
#   write.table(file=clip, sep = '\t', row.names = FALSE)
